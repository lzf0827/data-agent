...
def main() -> int:
    ...
    proto_group.add_argument("--proto-l3", dest="proto_l3", action="store_true", default=None, help="Enable the governed Proto-L3 planner for this run (default).")
    proto_group.add_argument("--no-proto-l3", dest="proto_l3", action="store_false", help="Use the legacy fixed pipeline for incident rollback.")
    args = parser.parse_args()
    if args.proto_l3 is not None:
        os.environ["PROTO_L3_ENABLED"] = "true" if args.proto_l3 else "false"
    ...
    try:
        result = pipeline.run(args.input.resolve(), intent, parse_confirmation(args.confirmed_mappings), prompt=f"分析 {args.sku} 在 {'、'.join(args.channels)} 最近 {args.months} 个月", plan_approved=args.proto_l3)
