...
class AnalysisStep(BaseModel):
    ...
    step_id: str = Field(pattern=r"^[a-z][a-z0-9_]{2,48}$")
    operation: Literal["inspect_workbook", "resolve_mapping", "extract_metrics", "validate_quality", "analyze_price_trend", "analyze_competitors", "search_official_evidence", "synthesize_insights", "generate_decision", "render_report"]
...
class AnalysisPlan(BaseModel):
    ...
    analysis_targets: list[AnalysisTarget] = Field(min_length=1, max_length=3)
    steps: list[AnalysisStep] = Field(min_length=1, max_length=20)
    source: Literal["deterministic", "agnes", "approved"]
...
class PlanExecutionState(BaseModel):
    ...
    status: Literal["planning", "awaiting_plan_approval", "executing", "replanning", "completed", "failed"]
    plan_hash: str
...
class ExecutionFeedback(BaseModel):
    ...
    step_id: str
    status: Literal["SUCCESS", "RETRYABLE_ERROR", "BLOCKED"]
    summary: str = Field(min_length=1, max_length=500)
    error_type: str | None = None
    provenance: list[str] = Field(default_factory=list)


def proto_l3_enabled() -> bool:
    # Proto-L3 is now the default governed path. Set PROTO_L3_ENABLED=false
    # for an explicit compatibility rollback during incident handling.
    return os.getenv("PROTO_L3_ENABLED", "true").strip().lower() in {"1", "true", "yes"}


def default_analysis_plan(intent: AnalysisIntent, objective: str = "") -> AnalysisPlan:
    steps = [
        AnalysisStep(step_id="inspect", operation="inspect_workbook", success_criteria=["semantic plan validated"]),
        AnalysisStep(step_id="mapping", operation="resolve_mapping", depends_on=["inspect"], approval_required=True, success_criteria=["mapping resolved or confirmed"]),
        AnalysisStep(step_id="extract", operation="extract_metrics", depends_on=["mapping"], success_criteria=["canonical records extracted"]),
        AnalysisStep(step_id="quality", operation="validate_quality", depends_on=["extract"], success_criteria=["quality gates pass"]),
        AnalysisStep(step_id="price", operation="analyze_price_trend", depends_on=["quality"]),
        AnalysisStep(step_id="competitors", operation="analyze_competitors", depends_on=["quality"]),
        AnalysisStep(step_id="evidence", operation="search_official_evidence", depends_on=["quality"]),
        AnalysisStep(step_id="insights", operation="synthesize_insights", depends_on=["price", "competitors", "evidence"]),
        AnalysisStep(step_id="decision", operation="generate_decision", depends_on=["insights"]),
        AnalysisStep(step_id="render", operation="render_report", depends_on=["quality", "decision"]),
    ]
    return AnalysisPlan(objective=objective or "完成受治理的 SKU 价格、销量与竞品分析", analysis_targets=intent.analysis_targets, steps=steps, source="deterministic")


def validate_analysis_plan(plan: AnalysisPlan, intent: AnalysisIntent) -> AnalysisPlan:
    if len(plan.steps) > int(os.getenv("PROTO_L3_MAX_STEPS", "20")):
        raise ValueError("Analysis plan exceeds the configured step limit")
    expected_targets = {(item.channel, item.primary_sku) for item in intent.analysis_targets}
    actual_targets = {(item.channel, item.primary_sku) for item in plan.analysis_targets}
    if actual_targets != expected_targets:
        raise ValueError("Analysis plan targets do not match the validated intent")
    ids = [item.step_id for item in plan.steps]
    if len(ids) != len(set(ids)):
        raise ValueError("Analysis plan contains duplicate step IDs")
    known = set(ids)
    for step in plan.steps:
        if step.operation not in PROTO_L3_STEPS or any(dep not in known or dep == step.step_id for dep in step.depends_on):
                raise ValueError(f"Invalid dependency or skipped gate in analysis step {step.step_id}")
    ...
    required = {"inspect_workbook", "resolve_mapping", "extract_metrics", "validate_quality", "render_report"}
    if not required.issubset(operations):
        raise ValueError("Analysis plan cannot skip structural, mapping, extraction, quality, or rendering gates")
    quality_ids = {item.step_id for item in plan.steps if item.operation == "validate_quality"}
    record_ids = {item.step_id for item in plan.steps if item.operation == "extract_metrics"}
    def dependency_operations(step_id: str) -> set[str]:
        seen: set[str] = set()
        ...
        return {by_id[item].operation for item in seen}
    for step in plan.steps:
        if step.operation == "render_report" and not (set(step.depends_on) & quality_ids):
            raise ValueError("Report rendering must depend on quality validation")
        if step.operation in {"analyze_price_trend", "analyze_competitors", "synthesize_insights", "generate_decision"} and not (dependency_operations(step.step_id) & {"validate_quality", "extract_metrics"}):
            raise ValueError(f"{step.operation} must depend on canonical records and quality validation")
    return plan
...
class AgnesClient:
    ...
    def generate_analysis_plan(self, prompt: str, intent: AnalysisIntent) -> AnalysisPlan:
        fallback = default_analysis_plan(intent, prompt)
        if not self.enabled:
            self._mark_fallback("analysis_plan")
            return fallback
        system = """Return exactly one JSON object matching AnalysisPlan. You are a governed planner for a SKU data agent. Choose only the registered operations supplied by the user. Never return SQL, Python, file paths, cell coordinates, numbers, metric values, mappings, URLs, or new tools. Preserve the exact validated analysis_targets. The plan must include inspect_workbook, resolve_mapping, extract_metrics, validate_quality, and render_report, with dependencies that form a DAG. Do not skip safety gates. Return JSON only."""
        payload = {
            "user_goal": prompt,
            "validated_intent": intent.model_dump(),
            "registered_operations": list(PROTO_L3_STEPS),
            "constraints": {"max_steps": int(os.getenv("PROTO_L3_MAX_STEPS", "20")), "human_confirmation_for_mapping": True},
        }
        try:
            result = self._chat_model(system, json.dumps(payload, ensure_ascii=False), AnalysisPlan, "analysis_plan")
            return validate_analysis_plan(result.model_copy(update={"source": "agnes", "planner_model": self.model}), intent)
        except Exception as exc:
            # A malformed model plan is a planner failure, not a data-plane
            # failure. Fall back to the deterministic governed DAG so L3 can
            # remain the default without accepting an unsafe dependency graph.
            self._mark_fallback("analysis_plan", exc)
            return fallback
...
class OfficialEvidenceProvider:
    ...
    def search(self, brand: str, model: str, periods: list[str], topics: list[str] | None = None) -> list[EvidenceRecord]:
        ...
        query = f"({' OR '.join(f'site:{domain}' for domain in domains)}) {brand} {model} {' OR '.join(periods)} {topic_text}".strip()
        try:
...
class InsightPipeline:
    def __init__(self, root: Path, template_path: Path) -> None:
        ...
        from insight_middleware import InsightMiddleware
        self.insight_middleware = InsightMiddleware(root)

    def build_analysis_plan(self, prompt: str, intent: AnalysisIntent) -> AnalysisPlan:
        return validate_analysis_plan(self.agnes.generate_analysis_plan(prompt, intent), intent)

    @staticmethod
    def _plan_trace(plan: AnalysisPlan, status: str = "planned", input_sha256: str | None = None) -> list[dict[str, Any]]:
        return [
            {
                ...
                "output_summary": "registered governed operation",
                "artifacts": [],
            }
            for step in plan.steps
        ]
    ...
    def _semantic_plan(self, adapter: WorkbookAdapter, *, labels_confirmed: bool = False) -> tuple[WorkbookSemanticPlan, WorkbookStructureManifest]:
        ...
        if cached is not None:
            try:
                ...
                # A stale/corrupt semantic cache must not take down preview.
                # Rebuild from the current workbook manifest instead.
                self.agnes._mark_fallback("semantic_plan_cache", exc)
                cached = None
            if cached is None:
                validated = None
            else:
        ...
        try:
            ...
            if draft is None:
                raise
            # The deterministic profiler is safe for known layouts. Preserve
            # its review flag so only genuinely uncertain layouts require approval.
            self.agnes._mark_fallback("semantic_plan", exc)
            plan = profiler.validate_plan(draft.model_copy(update={"prompt_version": prompt_version, "recognizer_model": None}), manifest)
    ...
    def run(
        ...
        *,
        ...
        proto_plan: AnalysisPlan | None = None
        if proto_l3_enabled():
            proto_plan = validate_analysis_plan(analysis_plan, intent) if analysis_plan is not None else self.build_analysis_plan(prompt or "完成 SKU 分析", intent)
        ...
        if semantic_plan.review_required and not semantic_labels_confirmed:
            return {
                ...
                **({"analysis_plan": proto_plan.model_dump(), "plan_execution": PlanExecutionState(status="executing", plan_hash=hashlib.sha256(proto_plan.model_dump_json().encode("utf-8")).hexdigest(), trace=self._plan_trace(proto_plan, "waiting_for_label_confirmation")).model_dump()} if proto_plan else {}),
            }
        if prepared["pending"]:
            return {
                ...
                "semantic_plan": semantic_plan.model_dump(),
                **({"analysis_plan": proto_plan.model_dump(), "plan_execution": PlanExecutionState(status="executing", plan_hash=hashlib.sha256(proto_plan.model_dump_json().encode("utf-8")).hexdigest(), trace=self._plan_trace(proto_plan, "waiting_for_mapping_confirmation")).model_dump()} if proto_plan else {}),
            }
        # Preserve the legacy Excel interaction order: workbook structure,
        # label confirmation and competitor selection happen before plan
        # approval. The plan is validated earlier, but no insight tool is
        # called before the existing pre-insight gates complete.
        if proto_plan and not plan_approved:
            return {
                "status": "awaiting_plan_approval",
                "intent": intent.model_dump(),
                "semantic_plan": semantic_plan.model_dump(),
                "analysis_plan": proto_plan.model_dump(),
                "plan_execution": PlanExecutionState(
                    status="awaiting_plan_approval",
                    plan_hash=hashlib.sha256(proto_plan.model_dump_json().encode("utf-8")).hexdigest(),
                    trace=self._plan_trace(proto_plan, "awaiting_plan_approval"),
                ).model_dump(),
            }
        ...
        try:
            for target in intent.analysis_targets:
                ...
                middleware_result = self.insight_middleware.run(
                ...
                evidence: list[EvidenceRecord] = middleware_result["evidence"]
                ...
                facts = {
                    ...
                    "deterministic_tool_results": middleware_result["analytics"],
                ...
                if proto_plan:
                    insight_agent_trace = [*self._plan_trace(proto_plan, "completed", source_hash), *insight_agent_trace]
                ...
                if proto_plan:
                    analysis["analysis_plan"] = proto_plan.model_dump()
                    analysis["plan_execution"] = PlanExecutionState(
                        status="completed",
                        plan_hash=hashlib.sha256(proto_plan.model_dump_json().encode("utf-8")).hexdigest(),
                        executed_steps=[item.step_id for item in proto_plan.steps],
                        trace=self._plan_trace(proto_plan, "completed", source_hash),
                    ).model_dump()
            ...
            manifest = self._build_manifest(temp_group, run_group, source_hash, intent, prepared["adapter_version"], semantic_plan)
            if proto_plan:
                manifest["analysis_plan"] = proto_plan.model_dump()
                manifest["plan_execution"] = PlanExecutionState(status="completed", plan_hash=hashlib.sha256(proto_plan.model_dump_json().encode("utf-8")).hexdigest(), executed_steps=[item.step_id for item in proto_plan.steps], trace=self._plan_trace(proto_plan, "completed", source_hash)).model_dump()
            write_json(temp_group / "run_manifest.json", manifest)
            ...
            return {"status": "completed", "run_id": run_group, "intent": intent.model_dump(), "semantic_plan": semantic_plan.model_dump(), "llm_status": self.agnes.status(), "results": results, **({"analysis_plan": proto_plan.model_dump(), "plan_execution": manifest["plan_execution"]} if proto_plan else {})}
        except Exception as exc:
